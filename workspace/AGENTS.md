...
> ⚠️ **每次任务调度和阶段完成，必须同步通知飞书群**

**飞书通知脚本**：`~/.openclaw/scripts/feishu-notify-full.sh`

**通知场景清单**：

| 类别 | 场景 | 触发时机 | 消息类型 |
|------|------|----------|----------|
| ⏸️ **待确认** | 项目立项 | 用户确认开始新项目 | 🔵 待确认 |
| ⏸️ **待确认** | PRD 待审核 | PRD 文档已完成 | 🔵 待确认 |
| ⏸️ **待确认** | 设计待审核 | 技术方案已完成 | 🔵 待确认 |
| ⏸️ **待确认** | 代码待Review | 代码开发完成 | 🔵 待确认 |
| ⏸️ **待确认** | 测试待验收 | 测试全部通过 | 🔵 待确认 |
| ⏸️ **待确认** | 发布待确认 | 版本准备就绪 | 🔵 待确认 |
| ✅ **完成** | PRD 完成 | PRD 编写完成 | 🟢 完成 |
| ✅ **完成** | 设计完成 | 技术/视觉设计完成 | 🟢 完成 |
| ✅ **完成** | 开发完成 | 代码开发完成 | 🟢 完成 |
| ✅ **完成** | 测试通过 | 测试全部通过 | 🟢 完成 |
| ✅ **完成** | 版本发布 | App 正式发布 | 🎉 发布 |
| ⚠️ **关键** | 项目立项 | 新项目正式启动 | 🚀 立项 |
| ⚠️ **关键** | 里程碑达成 | 完成关键阶段 | 🏆 里程碑 |
| ⚠️ **关键** | 遇到阻塞 | 任务无法继续 | ⚠️ 阻塞 |
| ⚠️ **关键** | 紧急问题 | 重大BUG或风险 | 🚨 紧急 |

**调用格式**：
```bash
~/.openclaw/scripts/feishu-notify-full.sh "🔵 PRD 已完成 - 待确认" "" "项目：XXX" "文档：PRD.md" "负责人：周小产"
```
...
2. 【进行通知】
   - 对话通知：💻 Developer 正在编写代码（3分钟）
   - 飞书通知：一般不需要
   - 示例："💻 Developer 正在编写代码（3分钟）"

3. 【完成通知】
   - 对话通知：✅ [Agent名称] 已完成 [任务类型]
   - 飞书通知：🔵 [任务] 已完成 - 待确认
   - 示例："✅ Product 已完成 PRD，请确认"
   - 飞书同步：
     ```bash
     ~/.openclaw/scripts/feishu-notify-full.sh "🔵 PRD 已完成 - 待确认" "" "项目：[项目名]" "文档：PRD.md" "负责人：周小产"
     ```

4. 【问题通知】
   - 对话通知：⚠️ [Agent名称] 遇到问题：[问题描述]
   - 飞书通知：⚠️ [任务] - 阻塞
   - 示例："⚠️ Developer 遇到编译错误，需要修复"
   - 飞书同步：
     ```bash
     ~/.openclaw/scripts/feishu-notify-full.sh "⚠️ [任务] - 阻塞" "" "项目：[项目名]" "问题：[问题]" "影响：[影响]" "负责人：[Agent名]"
     ```
...
#### Developer → Tester 交接清单

| 交接内容 | 说明 | 必填 |
|----------|------|------|
| test-config.json | App 路径、bundle-id、启动命令 | ✅ |
| 代码仓库地址 | GitHub 仓库地址 | ✅ |
| 核心功能列表 | 需要测试的功能 | ✅ |
